...
map_hospital_slots = {
    "hospital-department": "department=%s" if EXTRACT_SIMP else "department=LIKE(Department(%s))",
}

map_hospital_inputs = {
    "department": 'the %s department',
    'phone': 'the phone number is %s'
}
...
def extract_find_hospital(utterance, slots, context, general=None):
    return extract_find_domain(utterance, slots, context, 'Hospital', map_hospital_slots,
                               HOSPITAL_PREFIX, [], ['department', 'phone'], general)
